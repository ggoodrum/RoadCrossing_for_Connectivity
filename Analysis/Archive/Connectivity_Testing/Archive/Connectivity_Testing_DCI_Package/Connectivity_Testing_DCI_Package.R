...
DCI.calc <- function(nodefile, graphfile, mouth, edgemat, calc.DCIs = 0){
  ...
  for (k in 1:length(nodefile[,1])) {
    time.itsrt <- as_datetime(Sys.time())
    ...
    # Add processing time check to track
    ...
    time.itr  <- round(seconds_to_period(as.numeric(difftime(time.now, time.itsrt, units = 'secs'))), digits = 2) 
    print(paste0("Node: ", k, " of ", length(nodefile[,1]), 
                 '    Processing time: ', time.tot,
                 "    Iteration time: " , time.itr))
  }  
...
# --------------------------------------------------------------------- #
# 03. Chris Edge Example 
# ---------------------------------------------------------------------
# https://github.com/cbedge/DCI/blob/master/DCI_Calculaion_V2.txt
...
# Load in data
WC <- read.csv('testWC_V2.csv',
                         header = T)

# # OPTION: Convert all barriers to passable
# WC$PermUS <- 1
# WC$PermDS <- 1
...
# Calculate DCI
WCDCI <- DCI.calc(nodefile = WC.prep$Nodes, graphfile = WC.prep$graph, 
                  edgemat = WC.prep$edgematrix, mouth=6, calc.DCIs = 0)
